...
head(Air_Traffic_Passenger_Statistics_2)
str(Air_Traffic_Passenger_Statistics_2)
nrow(Air_Traffic_Passenger_Statistics_2)

# Set seed for reproducibility
set.seed(123)

# Generate random indices for data partitioning
indices <- sample(1:nrow(Air_Traffic_Passenger_Statistics_2))
train_indices <- indices[1:round(0.7 * length(indices))]
test_indices <- indices[(round(0.7 * length(indices)) + 1):length(indices)]

# Split the data into training and testing sets
train_data <- Air_Traffic_Passenger_Statistics_2[train_indices, ]
test_data <- Air_Traffic_Passenger_Statistics_2[test_indices, ]
